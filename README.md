...
- **BackBLAST.py** - A script that uses NCBI BLAST to search for gene clusters within a genome. Non-orthalagous genes are filtred out by identifying and extracting only Bidirectional Best BLAST Hits using a graph-based algorithm. The algorithm is illustrated below:
