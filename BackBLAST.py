...
def runBLAST(query, BLASTDBFile):
	...
	if query.startswith(">"):
		print "Back Blasting Hit Genome to Query Genome..."
...
def getTopHits(BLASTCSVOut): 
	...
	# Looks for first occurnce of each query accession. This should be the top hit. 
	# If ties occur includes these tied in the top hit list. (Ties should have the same score)
	for x in range(1, len(BLASTCSVOut)):
		if BLASTCSVOut[x][0] != currentQuery[0]:
			topHits.append(BLASTCSVOut[x])
			currentQuery = BLASTCSVOut[x]
		elif BLASTCSVOut[x][5] == currentQuery[5]: # Untested Jesus take the wheel...
			topHits.append(BLASTCSVOut[x])
...
def getQueryGenome(GenomesCSV, queryProtein):
	...
	for row in GenomesCSV:
		if row[1].strip() == queryProtein.strip(): 
			...
			break # Should remove break as its bad voodoo...
...
Hits = getTopHits(BLASTOut) # Finds top hits for each query protein and astors them in a list.
